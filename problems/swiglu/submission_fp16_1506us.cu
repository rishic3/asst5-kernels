...
torch::Tensor custom_kernel(
    ...
    auto W_fp16 = W.to(torch::kFloat16);
    auto V_fp16 = V.to(torch::kFloat16);
    auto b_fp16 = b.to(torch::kFloat16);
    auto c_fp16 = c.to(torch::kFloat16);
    
    auto W_fused = torch::empty({In, 2*H}, W.options());
    
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    
    size_t width_bytes = H * sizeof(half);
    size_t pitch_fused = 2 * H * sizeof(half);
    
    cudaMemcpy2DAsync(
        W_fused.data_ptr(), pitch_fused,
        W_fp16.data_ptr(), width_bytes,
        width_bytes, In,
        cudaMemcpyDeviceToDevice, stream
    );
    
    cudaMemcpy2DAsync(
        static_cast<char*>(W_fused.data_ptr()) + width_bytes, pitch_fused,
        V_fp16.data_ptr(), width_bytes,
        width_bytes, In,
        cudaMemcpyDeviceToDevice, stream
    );
    ...
    cublasLt_gemm_fp16(
        ...
        reinterpret_cast<const half*>(W_fused.data_ptr<at::Half>()),
